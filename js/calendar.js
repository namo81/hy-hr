...
if (dom)
{
	...
	document.write ("</table></div></div><div id='selectMonth' style='position:absolute;visibility:hidden;z-index:1096;width:72px;height=190px;' ><iframe src='about:blank' width='100%' height='100%' frameborder=0></iframe><div id='selectMonth2' style='position:absolute;left:0px;top:0px;z-index:1097' ></div></div><div id='selectYear' style='position:absolute;visibility:hidden;z-index:1096'></div><div id='selectMonth22' style='position:absolute;left:2px;top:2px;z-index:1098'></div></div><div id='selectYear2' style='position:absolute;visibility:hidden;z-index:1097'></div>");		
...
function constructMonth() {
	...
	if (!monthConstructed) {
		...
		for	(i=0; i<12;	i++) {
			...
			sHTML += "<tr class=box-font><td id='m" + i + "' onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer' onclick='monthConstructed=false;monthSelected=" + i + ";constructCalendar();popDownMonth();event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>";
		}
...
function constructMonth2() {
	...
	if (!monthConstructed2) {
		...
		for	(i=0; i<12;	i++) {
			...
			sHTML += "<tr class=box-font><td id='m" + i + "' onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer' onclick='monthConstructed2=false;monthSelected2=" + i + ";constructCalendar2();popDownMonth2();event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>";
		}
...
function popUpMonth() {
	...
	crossMonthObj.left =   parseInt(crossobj.left) + 110 +"px";     //parseInt(crossobj.left) + 50
...
function popUpMonth2() {
	...
	crossMonthObj2.left = parseInt(crossobj.left) + 320 +"px";
...
function incYear2() {
    ...
    for	(i=0; i<7; i++){
        ...
        if (newYear2==yearSelected2)
        { txtYear2 =	"&nbsp;<B>"	+ newYear2 +	"</B>&nbsp;"; }
        else
        { txtYear2 =	"&nbsp;" + newYear2 + "&nbsp;"; }
...
function constructYear2() {
	...
	if (!yearConstructed2) {
		...
		sHTML =	"<tr><td align='center'	onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='clearInterval(intervalID12);this.style.backgroundColor=\"\"' style='cursor:pointer'	onmousedown='clearInterval(intervalID12);intervalID12=setInterval(\"decYear2()\",30)' onmouseup='clearInterval(intervalID12)'>-</td></tr>";
		...
		for	(i=(yearSelected2-3); i<=(yearSelected2+3); i++) {
			...
			sHTML += "<tr><td id='y2" + j + "' onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='this.style.backgroundColor=\"\"' style='cursor:pointer' onclick='selectYear2("+j+");event.cancelBubble=true'>&nbsp;" + sName + "&nbsp;</td></tr>";
			j ++;
		}

		sHTML += "<tr><td align='center' onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='clearInterval(intervalID22);this.style.backgroundColor=\"\"' style='cursor:pointer' onmousedown='clearInterval(intervalID22);intervalID22=setInterval(\"incYear2()\",30)'	onmouseup='clearInterval(intervalID22)'>+</td></tr>";
...
function constructCalendar2() {
	...
	for	( datePointer=1; datePointer<=numDaysInMonth; datePointer++ )
	{
		dayPointer++;
		sStyle=styleAnchor;
		if ((datePointer==odateSelected2) &&	(monthSelected2==omonthSelected2)	&& (yearSelected2==oyearSelected2))
		{ sStyle+=styleLightBorder; }
		...
		if ((datePointer==dateNow)&&(monthSelected2==monthNow)&&(yearSelected2==yearNow))
		{ sHTML22 += "<td  align=right><a class='calendar-alink' "+dateMessage+" title=\"" + sHint + "\" style='"+sStyle+"' href='javascript:dateSelected2="+datePointer+";closeCalendar2();'><font color=blue>" + datePointer + "</font></a>";}
		else if	(dayPointer % 7 == (startAt * -1)+1)
		{
...
function ViewCalendar(ctl, ctl2,  format,act,DurCal,Flg, offsetx, offsety){
	...
	if(format == "none"){
		...
		document.getElementById("calendar").style.width="220px";
