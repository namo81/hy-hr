$.fn.baseGnb = function(option){
	...
	$d1list.on('mouseleave',function(){
		if($control == 'script') {
			...
			$(this).find($d2).removeClass($onclass);
		 }
	});
		...
		if($key == 9) { 
			if(e.shiftKey) {
				if($control == 'script'){
					...
					$(this).next($d2).removeClass($onclass);
				}
			} else { null; }
		}		
	});
...
$.fn.baselnb = function(option){
	...
	$d2list.each(function(){
		if($(this).hasClass($onclass)){
			$(this).parent().parent().addClass($onclass).show();
		}
	});
	...
	$d1btn.on('click',function(){
		...
		$(this).next($d2).stop().slideDown(300).parent().addClass('on');
	});
